...
**Coverage:** Part 1 and Part 2 combined, expanded in full depth for exam revision — every concept, every built-in variable, every function, and every operator is explained individually with its own working example(s).
...
AWK's entire design revolves around a simple but powerful idea: **treat input as a stream of records, and each record as a stream of fields.** This is what makes AWK so much shorter than equivalent C or Python code for text-processing tasks.
...
Already demonstrated in Section 2.4 (paragraph mode and custom separators). Additional example:
